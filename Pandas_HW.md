...
```python
#Trends:
    #Most players who purchase in game options are 18-25
    #Most players who purchase in game options are male
    #The most profitable items have relatively high sales
```
...
SN_count = df_players["SN"].nunique()
...
items = df_players["Item ID"].nunique()
ave_price = round(df_players["Price"].mean(),2)
purchase_count = df_players["Price"].count()
revenue = df_players["Price"].sum()

Purchasing_Analysis = pd.DataFrame({"Total Number of Purchases": [purchase_count],
...
unique_players = df_players.drop_duplicates(subset="SN", keep='first', inplace=False)
    
total_gender = unique_players["Gender"].count()
male = unique_players["Gender"].value_counts()["Male"]
female = unique_players["Gender"].value_counts()["Female"]
non_disclosed = total_gender - male - female
p_male = str(round((male/total_gender)*100,1))+"%"
p_female = str(round((female/total_gender)*100,1))+"%"
p_non_disclosed = str(round((non_disclosed/total_gender)*100,1))+"%"

gender_data = {"Count of Players": {"Male": male, "Female": female, "Non-Disclosed": non_disclosed},
              "Percentage of Players": {"Male": p_male, "Female": p_female, "Non-Disclosed": p_non_disclosed}}
...
df_male = df_players.loc[df_players["Gender"]=="Male"]
df_female = df_players.loc[df_players["Gender"]=="Female"]
df_non = df_players.loc[df_players["Gender"]=="Other \\/ Non-Disclosed"]

m_purchase_count = df_male["Gender"].count()
m_ave_price = round(df_male["Price"].mean(),2)
m_value = df_male["Price"].sum()
m_normal = round(revenue*(male/total_gender),2)
f_purchase_count = df_female["Gender"].count()
f_ave_price = round(df_female["Price"].mean(),2)
f_value = df_female["Price"].sum()
f_normal = round(revenue*(female/total_gender),2)
n_purchase_count = df_non["Gender"].count()
n_ave_price = round(df_non["Price"].mean(),2)
n_value = df_non["Price"].sum()
n_normal = round(revenue*((total_gender - male - female)/total_gender),)

purchase_data = {"Purchase Count": {"Male": m_purchase_count,"Female": f_purchase_count,"Non-Disclosed": n_purchase_count},
                "Average Price": {"Male": m_ave_price,"Female": f_ave_price,"Non-Disclosed": n_ave_price},
                "Total Value": {"Male": m_value,"Female": f_value,"Non-Disclosed": n_value},
               "Normalized Total": {"Male": m_normal,"Female": f_normal,"Non-Disclosed": n_normal}}
purchase_analysis = pd.DataFrame(purchase_data)
purchase_analysis_org = purchase_analysis[["Purchase Count", "Average Price", "Total Value", "Normalized Total"]]
purchase_analysis_org
```
...
```python
#Age Demographics: The below each broken into bins of 4 years (i.e. <10, 10-14, 15-19, etc.up to 45)
#Purchase Count; Average Purchase Price; Total Purchase Value; Normalized Totals (total purchase value*percent in each bin) 
...
ages = ['<10', '10-13', '14-17', '18-21', '22-25', '26-29', '30-33', '34-37', '38-41','42-46']

#Creating bins
df_players["Age Range"] = pd.cut(df_players["Age"], bins=bins, labels=ages)

#calculating the desired values for each bin
df_players_groups = df_players.groupby(["Age Range"],as_index=False)
age_purchase_count = df_players_groups[["Age"]].count()
age_ave_price = round(df_players_groups[["Price"]].mean(),2)
age_total = df_players_groups[["Price"]].sum()
#trying to get normalized totals:
    #number of people in each age group:
    #age_SN_count = df_players_groups[["SN"]].nunique
    #calculating the normalized total (percent in each age group * total revenue)
    #age_n_total = (age_SN_count/total_players)*revenue

#creating a dataframe for the desired values by age group
Age_Analysis = age_purchase_count.merge(age_ave_price, left_on='Age Range', right_on='Age Range', how='outer')
Age_Analysis = Age_Analysis.merge(age_total, left_on='Age Range', right_on='Age Range', how='outer')


#renaming the columns in the dataframe
Age_Analysis.rename(columns = {"Age": "Total Purchage by Age", 
                               "Price_x": "Average Purchase Price by Age", 
                               "Price_y": "Total Purchase Value by Age"})

#I could not figure out how to get the normalized totals column to add. Will solutions be provided for review? 
```
...
<table border="1" class="dataframe">
  <thead>
    <tr style="text-align: right;">
      <th></th>
      <th>Age Range</th>
      <th>Total Purchage by Age</th>
      <th>Average Purchase Price by Age</th>
...
#By SN I have count purchases (item ID), mean the price (Price), sum the price (Price), return highest frequency item(Item Id)

df_sn_grouped = df_players.groupby(["SN"], as_index = False)
sn_purchase_count = df_sn_grouped[["Item ID"]].count()
sn_ave_purchase = round(df_sn_grouped[["Price"]].mean(),2)
sn_total_purchase = df_sn_grouped[["Price"]].sum()

top_spenders = sn_purchase_count.merge(sn_ave_purchase, left_on='SN', right_on='SN', how='outer')
top_spenders = top_spenders.merge(sn_total_purchase, left_on='SN', right_on='SN', how='outer')
sorted_top_spenders = top_spenders.sort_values("Price_y", ascending=False)


sorted_top_spenders_rn = sorted_top_spenders.rename(columns = {"Item ID": "Purchase Count by SN", 
                                                              "Price_x": "Average Purchase Price by SN", 
                                                              "Price_y": "Total Purchases"})

sorted_top_spenders_rn.head()
```
...
<table border="1" class="dataframe">
  <thead>
    <tr style="text-align: right;">
      <th></th>
      <th>SN</th>
      <th>Purchase Count by SN</th>
      <th>Average Purchase Price by SN</th>
...
df_item_grouped = df_players.groupby(["Item ID"], as_index = False)
item_purchase_count = df_item_grouped[["SN"]].count()
item_price = df_item_grouped[["Price"]].mean()
item_total_purchase = df_item_grouped[["Price"]].sum()

top_items = item_purchase_count.merge(item_price, left_on='Item ID', right_on='Item ID', how='outer')
top_items = top_items.merge(item_total_purchase, left_on='Item ID', right_on='Item ID', how='outer')
sorted_top_items = top_items.sort_values("SN", ascending=False)


Most_Popular = sorted_top_items.rename(columns = {"SN": "Purchase Count by Item", 
                                                         "Price_x": "Item Price", 
                                                         "Price_y": "Total"})
...
#Most Profitable Items
    #Identify the 5 most profitable items by total purchase value, then list (in a table):
        #Item ID; Item Name; Purchase Count; Item Price; Total Purchase Value
sorted_top_items = top_items.sort_values("Price_y", ascending=False)

Most_Profitable = sorted_top_items.rename(columns = {"SN": "Purchase Count by Item", 
                                                         "Price_x": "Item Price", 
                                                         "Price_y": "Total Profit"})
